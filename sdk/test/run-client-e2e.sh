#!/bin/bash
# E2E del CLIENTE (SDK): levanta el stack, y desde la perspectiva del jugador
# registra cuenta, hace login, obtiene el ticket y genera los args de connect.
set -uo pipefail
...
cleanup() {
  ...
  sleep 2; rm -rf "$T" "$TICKETS" "$AUTH/accounts-client.json"
}
trap cleanup EXIT
...
QD_TICKET_DIR="$TICKETS" QD_ACCOUNTS_FILE="$AUTH/accounts-client.json" QD_SECRET="client-e2e" \
...
echo "=== 2. LAUNCHER del cliente (SDK) — perspectiva del jugador ==="
cd "$SDK"
QD_RUN="$T" E4COIN_CLI="$E4/e4coin-cli" node examples/launcher.mjs 1.2.3.4:27960
